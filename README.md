...
- **自研网格图 + BFS 寻路引擎**：敌人沿**格子边**移动并可自由选择最近的、可达的出口，铺路 / 拆路会触发**贪心寻路实时重算**，动态改道、堵路后自动换线。引擎（`engine.js` / `graph.js` / `renderer.js`）与框架解耦，可独立 `node --test` 单元测试。
- **完整前后端分离架构**：Vue 3 组件化 UI（Canvas 2D 渲染）+ Spring Boot REST API + MyBatis 持久化 + JWT 认证，职责清晰、易于扩展。
- **数据驱动的游戏数值**：方块 / 武器 / 防具 / 敌人 / 波次配置统一收敛到 `frontend/src/game/config.js`，调数值不动逻辑。
- **在线排行体系**：战绩按账号落库，排行榜每关每人取最高分，形成账号体系闭环。
- **覆盖完整游戏经济系统**：建造 / 击杀 / 金币路块 / 波次奖励 / 拆除全额返还，配**波次难度曲线**（血量按波数缩放、无尽模式滚动上升）。
